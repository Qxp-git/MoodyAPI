...
for cn = 1:length(cNames)
    ...
    if dim<0
       dim=3; % Default dimension value is 3. 
    end    
    data.dim=dim;
    ...
    fileList={'position','velocity','tangent','tension','strain','strainRate','modal','moment';
        'p'     ,   'v'    ,    'tan'  , 'T'     , 'eps'   , 'epsR'   ,'m','M'   };
    ...
    for vv=1:inArgs
        switch inputList{vv}
            ...
            case {'moment','mom'}
                try
                    data.M = getData('_moment','nodal',dim);
                catch e
                    disp('moment data not found')
                end
...
for rbn = 1:length(rbNames)
    ...
    % Read force data if present
    try 
	fForce=fopen(strcat(folderName,'/',objName,'_forces.dat'),'r');
	rawData=fread(fForce,'double');
	fclose(fForce);
	
	nColumns = length(rawData(:))/length(data.t);
	A=zeros(nColumns,length(data.t));
	A(:)=rawData;
	data.forces = A(2:end,:)';
    catch e
	disp(["No forces output found for " objName ". Add " objName ".printForces = 1  to inputfile and rerun to output forces."])	
    end


    % Collect info of body size and key features.     
    data.type = findKey(strcat(folderName,'/','setup.txt'),[objName '.type']);
    if strcmp(data.type,'cylinder')
        data.volume = findKey(strcat(folderName,'/','setup.txt'),[objName '.volume']);
        
        data.diameter = findKey(strcat(folderName,'/','setup.txt'),[objName '.diameter']);
        if data.diameter ~= -1
            data.height = data.volume/(0.25*data.diameter.^2*pi);
        else
            data.height = findKey(strcat(folderName,'/','setup.txt'),[objName '.height']);           
            data.diameter = sqrt(4*data.volume/(data.height*pi));
        end
    end
...
%% Read and store any variable
    ...
    function cNames = getObjectNames(folder,objType)
        ...
        [Ccell{1:length(C)}] = deal(C.name);
        C= Ccell; clear Ccell; 

        % C = extractfield(C,'name'); - needs toolbox
        ...
        for  nn=1:length(C)
            ...
            if nameLength>=targetLength && strcmp(C{nn}(1:targetLength),objType) 
                
                % Check for and truncate at substring:
                ind = strfind(C{nn},'_');
                if isempty(ind) % else truncate at ending
                    ind=strfind(C{nn},'.');
                end
                % Strip from ending:
                C{nn} = C{nn}(1:ind-1);                
